...
const renderProductList = () => {
  ...
  let htmlTbody = ``;
  products.forEach((p) => {
    htmlTbody += `<tr data-code ="${
      p["productCode"]
    }" data-product='${JSON.stringify(p)}'>
        <td>${p["productCode"]}</td>
        <td>${p["productName"]}</td>
        <td><button type="button" onclick="addToUpdate('${String(
          p["productCode"]
    ...
    if (tr.dataset.code == code) {
      ...
      if (btn.classList.contains("active")) {
        ...
        productDetail.innerHTML =
          `<div class="product-info-item" data-code ="${
            detail["productCode"]
                ...
                <div class="info-item-left">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" min="1" class="form-control" id="quantity" value="${Number(
                          detail["quantity"]
                        )}" name="quantity" required>
                    </div>
                    <div class="mb-3">
                        <label for="price" class="form-label">Price</label>
                        <input type="number" min="0" class="form-control" id="price" value="${Number(
                          detail["price"]
                        )}" name="price" required>
                    </div>
                    <div class="mb-3 input-file"></div>
                    <button type="button" class="btn btn-success" onclick="updateOne('${
                      detail["productCode"]
                    }')">Upload</button>
                </div>
                <div class="info-item-right">
                    <img src="\\${detail["image"]}" alt="Image">
                    <i class="fa-solid fa-circle-xmark" onclick="closeImg('${
                      detail["productCode"]
                    }')"></i>
                </div>
...
const updateOne = (code) => {
  ...
  details.forEach((d) => {
    if (d.dataset.code == code) {
      ...
      if(quantity<=0){
        showToast(`${code} quantity is invalid`)
        return;
      }
      if(price<0){
        showToast(`${code} price is invalid`)
        return;
      }
      let product = {
        productName,
...
const updateSelected = async () => {
  ...
  for (const d of details) {
    ...
    let image = d.querySelector("#image")?.files[0] || false;
    if(quantity<=0){
        showToast(`${d.dataset.code} quantity is invalid`)
        return;
      }
      if(price<0){
        showToast(`${d.dataset.code} price is invalid`)
        return;
      }
    let product = {
