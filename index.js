...
const generateCodes = async (products) => {
  try {
    ...
    products.forEach((p) => {
      ...
      if (list.length > 0) {
        list.forEach((item) => {
          if (item["productCode"]) {
            ...
            let itemId = parseInt(
              productCode.substring(item["category"].length + 1)
            );
      ...
      p["productCode"] =
        p.category + "-" + (maxId + 1).toString().padStart(4, "0");
...
app.get("/products/update/search", (req, res) => {
      ...
      try {
        client.connect().then((client) => {
          ...
          collection.insertMany(productsHaveCodes).then((productsInserted) => {
            ...
            res.json({
              message: "Insert products successfull",
              Products: data,
            });
            client.close();
          });
...
app.post("/products/addProductTemp", upload.single("image"), (req, res) => {
  ...
  if (index == -1) {
    ...
    if (oldSize != productsTemp.length) {
      res.json({
        message: "Add temp product successfull",
...
app.put("/products/updateSelected", async (req, res) => {
  ...
  try {
    ...
    if (productsToUpdateImg.length > 0) {
      await Promise.all(
        productsToUpdateImg.map(async (p) => {
          await collection
            .findOne({ productCode: p.productCode })
            .then((productFound) => {
              if (productFound.image) {
                fs.unlink("public\\" + productFound.image, (err) => {
                  if (err) {
...
app.put(
  "/products/loadDataToUpdate/:action",
  upload.single("image"),
  (req, res) => {
    if (Number(req.params.action) == 1) {
      let { productCode, productName, quantity, price } = JSON.parse(
        req.body.product
      );
      quantity = Number(quantity);
      price = Number(price);
    ...
    client.connect().then((client) => {
      ...
      collection
        ...
        .then((productsUpdated) => {
          ...
          res.json({
            message: "Reset price products successful",
            Products: data,
          });
          client.close();
        });
...
app.delete("/products/delete/:productCode", (req, res) => {
  ...
  if (productCode) {
    ...
    try {
      client.connect().then((client) => {
        ...
        const collection = db.collection(collectionName);
        collection
          .deleteOne({ productCode: productCode })
          .then((productDeleted) => {
...
app.delete("/products/deleteAll", (req, res) => {
  ...
  try {
    client.connect().then((client) => {
      const db = client.db(dbName);
      const collection = db.collection(collectionName);
      collection.deleteMany().then((productDeleted) => {
